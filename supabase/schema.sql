...
create table if not exists public.moderator_applications (
  ...
  -- Газар зүй / хаяг (browser geolocation)
...
-- 3-байрлалт нүүр (selfie) урсгал: зүүн/баруун талын селфи (урд нь selfie_face_url хэвээр)
alter table public.moderator_applications add column if not exists selfie_left_url text;
alter table public.moderator_applications add column if not exists selfie_right_url text;

-- Баримт урсгалын өргөтгөл: төрсний гэрчилгээ + эцэг эхийн үнэмлэх
-- Хүснэгт аль хэдийн байгаа бөгөөд баганагүй бол нэмнэ (idempotent)
alter table public.moderator_applications add column if not exists birth_certificate_url text;
alter table public.moderator_applications add column if not exists parent_id_url text;
alter table public.moderator_applications add column if not exists parent_id_owner text;

-- Баримтын төрөл: өргөдөл гаргагч өөрийн иргэний үнэмлэх ('id') ЭСВЭЛ
-- төрсний гэрчилгээ + эцэг/эхийн аль нэгний үнэмлэх ('birth-cert') — аль нэгээр нь
-- баталгаажуулна. Төрсний гэрчилгээгээр баталгаажсан үед л эцэг/эхийн үнэмлэх заавал.
alter table public.moderator_applications add column if not exists identity_document_type text not null default 'id';
alter table public.moderator_applications drop constraint if exists moderator_applications_identity_document_type_check;
alter table public.moderator_applications add constraint moderator_applications_identity_document_type_check
  check (identity_document_type in ('id','birth-cert'));

-- parent_id_owner нэртэй constraint (idempotent): ээж/аав гэсэн 2 утга л зөвшөөрнө
alter table public.moderator_applications drop constraint if exists moderator_applications_parent_id_owner_check;
alter table public.moderator_applications add constraint moderator_applications_parent_id_owner_check
  check (parent_id_owner in ('father','mother'));
