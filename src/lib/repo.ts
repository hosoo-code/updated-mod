...
// anon-д зөвшөөрөгдсөн moderator баганууд (schema.sql дахь grant-тай таарна).
// `user_id`, `address`, `parent_phone` гэх мэт нь НУУЦ — public query-д сонгохгүй,
// эс бөгөөс column-level privilege асуудал үүсгэнэ (PGRST/permission denied).
const PUBLIC_MOD_COLS =
  "id, full_name, nickname, avatar_url, facebook_url, phone, location_text, location_status, became_moderator_at, verification_status, verified_at, last_weekly_verification_at, next_weekly_verification_at, is_active, is_public, created_at, moderator_groups(is_primary, groups(*))";
...
export async function getPublicModerators(): Promise<PublicModerator[]> {
  ...
  const { data, error } = await sb
    .from("moderators")
    .select(PUBLIC_MOD_COLS as "id, full_name, nickname, avatar_url, facebook_url, phone, location_text, location_status, became_moderator_at, verification_status, verified_at, last_weekly_verification_at, next_weekly_verification_at, is_active, is_public, created_at, moderator_groups(is_primary, groups(*))")
    .eq("is_public", true)
    .eq("is_active", true);
...
export async function getActiveGroups(): Promise<Group[]> {
  ...
  const { data, error } = await sb
    .from("groups")
    .select("*, moderator_groups(is_primary, moderators(id, full_name, nickname, is_active, is_public, verification_status, created_at))")
    .eq("is_active", true)
    .eq("is_hidden", false)
    .order("sort_order");
  if (error) throw new Error(error.message);
...
export async function createVerificationRequest(
  ...
  if (isDemoMode()) {
    const reusable = [...demoStore.requests]
      .filter(
        (r) =>
          r.userId === userId &&
          r.documentType === input.documentType &&
          (r.status === "resubmit_requested" || r.status === "draft")
      )
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
    if (reusable) {
      demoUpdateRequest(reusable.id, {
        status: "draft",
  ...
  const sb = await createServiceClient();
  const reusableQuery = await sb
    .from("verification_requests")
    .select("id")
    .eq("user_id", userId)
    .eq("document_type", input.documentType)
    .in("status", ["resubmit_requested", "draft"])
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (reusableQuery.error) throw new Error(reusableQuery.error.message);
  if (reusableQuery.data?.id) {
    const { data, error } = await sb
      .from("verification_requests")
      .update({
        status: "draft",
...
export async function addVerificationDocument(
  ...
  if (isDemoMode()) {
    ...
    if (added && doc.faceResult && doc.documentType === "face") {
      ...
      if (req) {
        req.faceResult = {
          ...
          note: "Client camera evidence — server баталгаажаагүй.",
        };
      }
    }
    return added;
  ...
  const requestOwner = await getVerificationById(requestId);
  const replacement = {
    request_id: requestId,
    user_id: requestOwner?.userId ?? null,
  ...
  const query = existing?.id
    ? sb.from("verification_documents").update(replacement).eq("id", existing.id).select("*").single()
    : sb.from("verification_documents").insert(replacement).select("*").single();
  const { data, error } = await query;
  if (error) throw new Error(error.message);
  if (doc.faceResult && doc.documentType === "face") {
    // Client evidence-г хадгалсан ч authoritative pass/liveness гэж тэмдэглэхгүй.
    // Production request дээр зөвхөн advisory, forced-false төлөвөөр хадгална.
    await sb
      .from("verification_requests")
      .update({
        face_result: {
...
export async function listLocationsAdmin(): Promise<LocationRecord[]> {
  ...
  if (error) throw new Error(error.message);
  const rows = data ?? [];
  // moderator-ын нэрийг тусад нь багц query-аар шийднэ (join-д FK байхгүй тул)
  const userIds = [...new Set(rows.map((r) => str(r.user_id)).filter(Boolean))] as string[];
  ...
  if (userIds.length > 0) {
    ...
    if (!modErr) {
      for (const m of mods ?? []) {
        ...
        if (uid) nameByUser.set(uid, str(m.nickname) ?? str(m.full_name) ?? "");
      }
    }
  }
  return rows.map((row) => {
    const rec = mapLocation(row);
    const name = nameByUser.get(str(row.user_id) ?? "");
    if (name) rec.moderatorName = name;
    return rec;
  });
}
...
export async function listActiveLocationsAdmin(): Promise<ActiveLocationShare[]> {
  ...
  // moderator-ын нэрийг тусад нь багц query-аар шийднэ (schema дээр moderator_name байхгүй)
  ...
  return [...byUser.values()].map((row) => {
    ...
    return {
      ...
      moderatorName: nameByUser.get(str(row.user_id) ?? "") ?? "Хэрэглэгч",
...
function mapModeratorApplication(row: Row): ModeratorApplicationData {
  ...
  return {
    ...
    educationEmployment: {
      ...
      schoolName: str(row.school_name) ?? "",
      schoolGrade: str(row.school_grade) ?? "",
      teacherName: str(row.teacher_name) ?? "",
      teacherPhone: str(row.teacher_phone) ?? "",
      teacherFacebookLink: str(row.teacher_facebook_link) ?? "",
      isEmployed: row.is_employed === true,
      workplaceName: str(row.workplace_name) ?? "",
      workplaceLocation: str(row.workplace_location) ?? "",
      directorPhone: str(row.director_phone) ?? "",
    },
    ...
    selfieLeftUrl: str(row.selfie_left_url),
    selfieRightUrl: str(row.selfie_right_url),
    ...
    birthCertificateUrl: str(row.birth_certificate_url),
    parentIdUrl: str(row.parent_id_url),
    parentIdOwner: (str(row.parent_id_owner) as ModeratorApplicationData["parentIdOwner"]) ?? null,
    identityDocumentType:
      (str(row.identity_document_type) as ModeratorApplicationData["identityDocumentType"]) ?? "id",
    father: {
...
export async function updateApplicationImages(
  ...
  patch: {
    ...
    parentIdUrl?: string | null;
    identityDocumentType?: "id" | "birth-cert";
  }
): Promise<ModeratorApplicationData | null> {
  ...
  if (patch.selfieLeftUrl !== undefined) update.selfie_left_url = patch.selfieLeftUrl;
  if (patch.selfieRightUrl !== undefined) update.selfie_right_url = patch.selfieRightUrl;
  if (patch.birthCertificateUrl !== undefined) update.birth_certificate_url = patch.birthCertificateUrl;
  if (patch.parentIdUrl !== undefined) update.parent_id_url = patch.parentIdUrl;
  if (patch.identityDocumentType !== undefined)
    update.identity_document_type = patch.identityDocumentType;
  // Анкет draft/editable биш бол 0 мөр шинэчлэгдэнэ — .single() нь PGRST116
  // throw хийгээд хуудсыг унагаахгүйн тулд .maybeSingle() хэрэглэнэ.
  const { data, error } = await sb
    .from("moderator_applications")
    .update(update)
    .eq("id", appId)
...
export async function submitModeratorApplication(
  ...
  payload: {
    ...
    facebookLink: string;
    phoneNumbers: string[];
    educationEmployment: EducationEmploymentInfo;
    ...
    birthCertificateUrl: string | null;
    parentIdUrl: string | null;
    parentIdOwner: "father" | "mother" | null;
    identityDocumentType: "id" | "birth-cert";
    father: ParentInfo;
    mother: ParentInfo;
  ...
  // Төрсний гэрчилгээний баримтыг админ өөрөө харж шалгана — OCR автомат уншилт байхгүй.
  ...
  const { data, error } = await sb
    ...
    .update({
      ...
      is_student: payload.educationEmployment.isStudent,
      school_name: payload.educationEmployment.schoolName,
      school_grade: payload.educationEmployment.schoolGrade,
      teacher_name: payload.educationEmployment.teacherName,
      teacher_phone: payload.educationEmployment.teacherPhone,
      teacher_facebook_link: payload.educationEmployment.teacherFacebookLink,
      is_employed: payload.educationEmployment.isEmployed,
      workplace_name: payload.educationEmployment.workplaceName,
      workplace_location: payload.educationEmployment.workplaceLocation,
      director_phone: payload.educationEmployment.directorPhone,
      id_card_front_urls: payload.idCardFrontUrls,
      id_card_back_urls: payload.idCardBackUrls,
      selfie_face_url: payload.selfieFaceUrl,
      selfie_left_url: payload.selfieLeftUrl ?? "",
      selfie_right_url: payload.selfieRightUrl ?? "",
      birth_certificate_url: payload.birthCertificateUrl ?? "",
      parent_id_url: payload.parentIdUrl ?? "",
      parent_id_owner: payload.parentIdOwner,
    ...
    .maybeSingle();
  if (error) throw new Error(error.message);
  if (!data) throw new Error("Илгээх боломжтой анкет олдсонгүй.");
