...
import { runDocumentRead } from "@/lib/document-reader";
import { buildPendingDocumentRead } from "@/lib/ocr-match";
import type {
  ActiveLocationShare,
  AddressHistoryEntry,
  AuditEntry,
...
function mapModeratorApplication(row: Row): ModeratorApplicationData {
  ...
  return {
    ...
    documentReadResult: (row.document_read_result as DocumentReadResult | null) ?? null,
    father: {
...
export async function submitModeratorApplication(
  ...
  // Төрсний гэрчилгээ + эцэг эхийн тааруулалт (OCR) — нотолгоо admin-д.
  // Түлхүүргүй/алдаа үед `pending` буцаана (fail-open) — submit хэзээ ч бүтэлгүйтэхгүй.
  let docReadResult: DocumentReadResult;
  if (payload.birthCertificateUrl) {
    try {
      docReadResult = await runDocumentRead({
        birthCertificateKey: payload.birthCertificateUrl,
        father: payload.father,
        mother: payload.mother,
      });
    } catch {
      docReadResult = buildPendingDocumentRead(payload.birthCertificateUrl, "OCR уншилт амжилтгүй — admin гараар шалгана уу.");
    }
  } else {
    docReadResult = buildPendingDocumentRead("", "Төрсний гэрчилгээ аваагүй байна.");
  }
  const docReadNote =
    docReadResult.matchStatus === "mismatch"
      ? `Төрсний гэрчилгээ эцэг эхийн мэдээлэлтэй таарахгүй байна: ${docReadResult.issues.join(" ")}`
      : null;
  ...
  // OCR/тааруулалтын нотолгоог document_read_result баганад хадгална — ижил
  // fail-safe загвар: багана байхгүй бол үндсэн submit амжилтгүй болохгүй
  if (app && payload.birthCertificateUrl) {
    try {
      await sb
        .from("moderator_applications")
        .update({ document_read_result: docReadResult })
        .eq("id", app.id)
        .select("document_read_result")
        .single();
    } catch {
      // Багана суулгаагүй байж болно — үндсэн бүртгэл аль хэдийн амжилттай
    }
