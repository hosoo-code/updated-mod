...
export async function uploadVerificationImage(
  ...
  // Upload дууссаны дараа server-side Python detector ажиллуулна.
  // Detector unavailable бол admin review fallback буцаана; upload өөрөө амжилттай хэвээр.
  const analysisRes = await fetch(`/api/verifications/${requestId}/analyze`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  const analysisJson = await analysisRes.json();
  if (!analysisRes.ok || !analysisJson.ok) {
    throw new Error(analysisJson.error ?? "Зургийг шалгахад алдаа гарлаа.");
  }
...
/**
 * Анкетын зураг upload — R2-д шууд (signed) upload, дараа нь серверт бүртгүүлнэ.
 * Slot: id-front-0, id-back-0, selfie, birth-certificate, parent-id
 */
export async function uploadApplicationImage(
  ...
  const regRes = await fetch("/api/apply/register-image", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ applicationId, objectKey, slot }),
  });
  const regJson = await regRes.json();
  if (!regJson.ok) throw new Error(regJson.error ?? "Зураг бүртгэхэд алдаа гарлаа.");
