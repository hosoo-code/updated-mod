...
/** Server-only binary read for trusted processing services. */
export async function readR2Object(objectKey: string): Promise<Uint8Array> {
  const env = serverEnv();
  const result = await client().send(
    new GetObjectCommand({ Bucket: env.r2BucketName, Key: objectKey })
  );
  if (!result.Body) throw new Error("R2 object хоосон байна.");
...
/**
 * Модератор анкетын зургийн object key — олон алхамт анкетын үнэмлэх (урд/ард) + селфи.
