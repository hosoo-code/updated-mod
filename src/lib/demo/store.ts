...
export function demoUpdateApplicationImages(
  ...
  if (patch.selfieLeftUrl !== undefined) app.selfieLeftUrl = patch.selfieLeftUrl;
  if (patch.selfieRightUrl !== undefined) app.selfieRightUrl = patch.selfieRightUrl;
  if (patch.birthCertificateUrl !== undefined) app.birthCertificateUrl = patch.birthCertificateUrl;
  if (patch.parentIdUrl !== undefined) app.parentIdUrl = patch.parentIdUrl;
  if (patch.identityDocumentType !== undefined) app.identityDocumentType = patch.identityDocumentType;
...
export function demoSubmitModeratorApplication(
  ...
  app.educationEmployment = payload.educationEmployment;
  ...
  app.selfieLeftUrl = payload.selfieLeftUrl;
  app.selfieRightUrl = payload.selfieRightUrl;
  app.birthCertificateUrl = payload.birthCertificateUrl;
  app.parentIdUrl = payload.parentIdUrl;
  app.parentIdOwner = payload.parentIdOwner;
  app.identityDocumentType = payload.identityDocumentType;
...
export function demoAddDocument(
  ...
  const req = demoStore.requests.find((r) => r.id === requestId);
  if (!req) return null;
  const existing = req.documents.find(
    (item) => item.documentType === doc.documentType && item.status === "uploaded"
  );
  const full: VerificationDocument = existing
    ? {
        ...existing,
        ...doc,
        status: "uploaded",
