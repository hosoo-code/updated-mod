...
export function demoSubmitModeratorApplication(
  ...
  // Демо горимд бодит OCR backend алга → provider "none", matchStatus "pending",
  // admin гараар төрсний гэрчилгээ/эцэг эхийн мэдээллийг тааруулна (fail-open)
  if (payload.birthCertificateUrl) {
    app.documentReadResult = buildPendingDocumentRead(payload.birthCertificateUrl);
  } else {
    app.documentReadResult = buildPendingDocumentRead("", "Төрсний гэрчилгээ аваагүй байна.");
  }
