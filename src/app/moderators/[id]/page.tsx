...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return { title: "Moderator олдсонгүй" };
  }
}
...
export default async function ModeratorProfilePage({ params }: PageProps) {
  const { id } = await params;
  let m: Awaited<ReturnType<typeof getPublicModerator>> = null;
  try {
    m = await getPublicModerator(id);
  } catch (err) {
    // Production-д Supabase/schema алдаа гарвал хуудсыг унагаахгүй — notFound харуулна.
    console.error("[ModeratorProfilePage] Data fetch failed:", err);
  }
  if (!m) notFound();
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 sm:px-6 sm:py-10">
        ...
        <Card className="mt-6 overflow-hidden">
          <div className="relative h-24 bg-zinc-100 dark:bg-zinc-900 sm:h-28">
            <div className="absolute inset-0 bg-[radial-gradient(80%_120%_at_20%_0%,rgba(113,113,122,0.18),transparent)]" />
          </div>
          <CardContent className="-mt-10 p-4 sm:p-6">
