...
export default async function AdminReviewPage({ params }: PageProps) {
  ...
  let locations: Awaited<ReturnType<typeof listLocationsAdmin>> = [];
  try {
    [ips, locations] = await Promise.all([
      listIpsAdmin(),
      listLocationsAdmin(),
    ]);
  } catch (err) {
    // Production-д Supabase/RLS алдаа гарвал review хуудсыг унагаахгүй — хоосон жагсаалт буцаана.
    console.error("[AdminReviewPage] Data fetch failed:", err);
  }
