...
export function ApplicationReview({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Төрсний гэрчилгээ OCR + эцэг эхийн тааруулалтын нотолгоо */}
      <Section icon={<ScanText className="h-4 w-4" />} title="Төрсний гэрчилгээ OCR / эцэг эхийн тааруулалт">
        {current.documentReadResult ? (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <BadgeL label="OCR provider" value={current.documentReadResult.provider} />
              <BadgeL label="Тааруулалтын төлөв" value={matchStatusLabel(current.documentReadResult.matchStatus)} />
              <EvidenceChip
                label="Эцэг (тааруулалт)"
                ok={current.documentReadResult.father.matched}
                value={`${Math.round(current.documentReadResult.father.score * 100)}%`}
              />
              <EvidenceChip
                label="Эх (тааруулалт)"
                ok={current.documentReadResult.mother.matched}
                value={`${Math.round(current.documentReadResult.mother.score * 100)}%`}
              />
            </div>
            {current.documentReadResult.issues.length > 0 ? (
              <ul className="space-y-1 text-xs text-amber-600 dark:text-amber-300">
                {current.documentReadResult.issues.map((issue, i) => (
                  <li key={i} className="flex items-start gap-1.5">
                    <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {issue}
                  </li>
                ))}
              </ul>
            ) : null}
            {current.documentReadResult.extractedText ? (
              <details className="rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900">
                <summary className="cursor-pointer text-xs font-medium text-zinc-600 dark:text-zinc-300">
                  OCR-ийн гаргасан текст харах
                </summary>
                <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-xs text-zinc-600 dark:text-zinc-300">
                  {current.documentReadResult.extractedText}
                </pre>
              </details>
            ) : (
              <p className="text-xs text-zinc-400">OCR текст уншигдаагүй — гараар шалгана уу.</p>
            )}
          </div>
        ) : (
          <p className="text-sm text-zinc-400">OCR үр дүн алга.</p>
        )}
      </Section>
