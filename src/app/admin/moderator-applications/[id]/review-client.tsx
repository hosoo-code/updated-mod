...
const FRONT_LABELS: string[] = ["Иргэний үнэмлэх урд тал"];
const BACK_LABELS: string[] = ["Иргэний үнэмлэх ард тал"];
...
export function ApplicationReview({
  ...
  return (
    <div className="space-y-6">
      ...
      <Section icon={<Users className="h-4 w-4" />} title="Сургууль, ажил">
        <Row label="Сурдаг эсэх" value={current.educationEmployment.isStudent ? "Тийм" : "Үгүй"} />
        {current.educationEmployment.isStudent ? <>
          <Row label="Сургууль" value={current.educationEmployment.schoolName} />
          <Row label="Анги / курс" value={current.educationEmployment.schoolGrade} />
          <Row label="Багш" value={current.educationEmployment.teacherName} />
          <Row label="Багшийн утас" value={current.educationEmployment.teacherPhone} />
          <Row label="Багшийн Facebook" value={current.educationEmployment.teacherFacebookLink} />
        </> : null}
        <Row label="Ажилладаг эсэх" value={current.educationEmployment.isEmployed ? "Тийм" : "Үгүй"} />
        {current.educationEmployment.isEmployed ? <>
          <Row label="Ажлын газар" value={current.educationEmployment.workplaceName} />
          <Row label="Ажлын байршил" value={current.educationEmployment.workplaceLocation} />
          <Row label="Захирал / менежерийн утас" value={current.educationEmployment.directorPhone} />
        </> : null}
      </Section>

      {/* Иргэний үнэмлэх / баримт — баримтын төрлөөс хамаарч харагдана */}
      <Section icon={<Fingerprint className="h-4 w-4" />} title={current.identityDocumentType === "birth-cert" ? "Баримт (төрсний гэрчилгээ горим)" : "Иргэний үнэмлэх"}>
        <div className="rounded-lg border border-brand-500/30 bg-brand-500/5 px-3 py-2 text-xs font-medium text-zinc-700 dark:text-zinc-300">
          {current.identityDocumentType === "birth-cert"
            ? "Төрсний гэрчилгээгээр баталгаажуулсан — өөрийн үнэмлэх байхгүй. Эцэг/эхийн үнэмлэхийг доор шалгана."
            : "Өөрийн иргэний үнэмлэхээр баталгаажуулсан — эцэг/эхийн үнэмлэх шаардлагагүй."}
        </div>
        {current.identityDocumentType === "id" ? (
          <>
          ...
          <p className="text-sm text-zinc-400">— (төрсний гэрчилгээний горим)</p>
        )}
      </Section>

      {/* Селфи — 3 байрлалт нүүр (урд/зүүн/баруун) */}
      <Section icon={<ScanFace className="h-4 w-4" />} title="Селфи (амьд нүүр)">
        <div className="flex flex-wrap items-start gap-3">
          {current.selfieFaceUrl ? (
            <SecureImage objectKey={current.selfieFaceUrl} label="Селфи урд" rounded />
          ) : (
            <p className="text-sm text-zinc-400">—</p>
          )}
          {current.selfieLeftUrl ? (
            <SecureImage objectKey={current.selfieLeftUrl} label="Селфи зүүн" />
          ) : null}
          {current.selfieRightUrl ? (
            <SecureImage objectKey={current.selfieRightUrl} label="Селфи баруун" />
          ) : null}
        </div>
        ...
        {current.faceResult ? (
          <div className="mt-3 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
              ...
              />
              <EvidenceChip
                label="Байрлалын зураг"
                ok={(current.faceResult.checks.poseCaptures ?? 0) >= 3}
                value={`${current.faceResult.checks.poseCaptures ?? 0}/3`}
              />
              <EvidenceChip
                label="Нүүр халхалсан"
                ok={!current.faceResult.checks.occluded}
                value={current.faceResult.checks.occluded ? "Тийм ⚠️" : "Үгүй"}
              />
            </div>
      ...
      {/* Төрсний гэрчилгээ */}
      <Section icon={<FileText className="h-4 w-4" />} title="Төрсний гэрчилгээ">
        <ThumbStrip label="Төрсний гэрчилгээ (бүтэн)" objectKey={current.birthCertificateUrl ?? undefined} />
      </Section>

      {/* Эцэг эхийн иргэний үнэмлэх — зөвхөн төрсний гэрчилгээний горимд */}
      {current.identityDocumentType === "birth-cert" ? (
        <>
          <Section icon={<Users className="h-4 w-4" />} title="Эцэг эхийн иргэний үнэмлэх">
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
              Төрсний гэрчилгээний эзэн = өргөдөл гаргагч байх ёстой. Доорх үнэмлэх нь доор
              бөглөсөн эцэг эсвэл эхийнх байх ба тухайн хүн төрсний гэрчилгээнд бичигдсэн
              эцэг/эх байх шаардлагатай. Зургийг харьцуулж шалгана уу.
            </div>
            <ThumbStrip
              label={`Эцэг/эх үнэмлэх (урд) — ${current.parentIdOwner === "mother" ? "ээж" : "аав"}`}
              objectKey={current.parentIdUrl ?? undefined}
            />
          </Section>
