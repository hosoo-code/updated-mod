...
export function ApplyWizard({
  ...
  const activeStepKeys = useMemo(() => {
    const base = ["info", "phone", "idcard", "bank", "address", "selfie", "review", "done"];
    if (identityType === "birth-cert") {
      return ["info", "phone", "idcard", "birthcert", "parents", "bank", "address", "selfie", "review", "done"];
    }
    return base;
  }, [identityType]);
  ...
  const next = () => {
    ...
    if (currentStepKey === "phone" && !phonesOk) return toast.error("Утасны дугаараа зөв оруулна уу (дор хаяж нэг).");
