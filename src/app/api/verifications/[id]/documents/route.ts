...
export const POST = withApi(async (req: NextRequest, { params }: Params) => {
  ...
  // Хуучин client-ийн response-д poseCaptures/occluded байхгүй байж болно — default-оор normalize
  const faceResult: FaceCheckResult | null = parsed.data.faceResult
    ? {
        ...parsed.data.faceResult,
        checks: {
          ...parsed.data.faceResult.checks,
          poseCaptures: parsed.data.faceResult.checks.poseCaptures ?? 0,
          occluded: parsed.data.faceResult.checks.occluded ?? false,
        },
      }
    : null;
  const expectedKey = buildObjectKey(user.id, id, parsed.data.documentType);
  if (parsed.data.objectKey !== expectedKey) {
    return err("Баримтын object key буруу байна.", 400);
  }
