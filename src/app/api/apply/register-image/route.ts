...
export const POST = withApi(async (req) => {
  ...
  const expectedKey = buildApplicationObjectKey(user.id, applicationId, slot);
  ...
  } else if (slot === "selfie-left") {
    updated = await updateApplicationImages(user.id, applicationId, { selfieLeftUrl: objectKey });
  } else if (slot === "selfie-right") {
    updated = await updateApplicationImages(user.id, applicationId, { selfieRightUrl: objectKey });
  } else if (slot === "birth-certificate") {
    updated = await updateApplicationImages(user.id, applicationId, { birthCertificateUrl: objectKey });
  } else if (slot === "parent-id") {
    updated = await updateApplicationImages(user.id, applicationId, { parentIdUrl: objectKey });
