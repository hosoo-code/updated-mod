  ...
  return (
    <Card>
      <CardHeader className="items-center text-center">
        <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-100 text-zinc-950 dark:bg-white/10 dark:text-white">
          {confirmed ? <ShieldCheck className="h-6 w-6" /> : <UserPlus className="h-6 w-6" />}
        </div>
            ...
            <div className="relative">
              ...
              {password ? (
                <div className="mt-2 space-y-2" aria-live="polite">
                  <div className="flex gap-1" aria-label={`Нууц үгийн хүч: ${passwordScore}/3`}>
                    {[0, 1, 2].map((index) => (
                      <span key={index} className={`h-1 flex-1 rounded-full ${index < passwordScore ? "bg-zinc-900 dark:bg-white" : "bg-zinc-200 dark:bg-white/10"}`} />
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-zinc-400">
                    {passwordChecks.map((check) => <span key={check.label} className={check.ok ? "text-zinc-800 dark:text-zinc-200" : ""}>{check.ok ? "✓" : "○"} {check.label}</span>)}
                  </div>
                </div>
              ) : null}
              <button
                ...
                className="focus-ring absolute bottom-1.5 right-2 flex h-8 w-8 items-center justify-center rounded-lg text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-950 dark:hover:bg-white/10 dark:hover:text-white"
              >
