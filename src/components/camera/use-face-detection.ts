      ...
      return {
        ...
        async createFaceLandmarker() {
          try {
            ...
            /**
             * Нүүр халхалсан эсэх (маск/гар/үс) — зөвхөн landmark-al-ийн үед.
             * Heuristic: нүдний 6+6 индекс бүтэн байгаа ч, амны өргөн (81↔308 x-зай)
             * нүүрний өргөнөөс маш бага (<0.32 харьцаа) бол амны хэсэг халхлагдсан.
             */
            const detectCovered = (lm: { x: number; y: number }[]): boolean => {
              if (!lm || lm.length < 380) return false;
              // Нүдний цэгүүд frame-д бүтэн байгаа эсэх
              let eyesFull = true;
              for (const idx of [...LEFT_EYE, ...RIGHT_EYE]) {
                const p = lm[idx];
                if (!p || p.x < 0 || p.y < 0 || p.x > 1 || p.y > 1) eyesFull = false;
              }
              if (!eyesFull) return false; // нүд ч бүрэн харагдахгүй — энэ тохиолдол "none" guidance
              const mouthL = lm[81];
              const mouthR = lm[308];
              if (!mouthL || !mouthR) return false;
              // Нүүрний өргөн (хэвтээ хэлхээний maxX-minX)
              let minX = 1, maxX = 0;
              ...
              const faceW = Math.max(1e-4, maxX - minX);
              const mouthRatio = Math.abs(mouthR.x - mouthL.x) / faceW;
              return mouthRatio < 0.32;
            };
