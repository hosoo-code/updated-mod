"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import { ArrowLeft, Camera, CheckCircle2, RefreshCcw, ScanFace, Loader2 } from "lucide-react";
import { useCamera, cameraErrorMessage, type CameraStatus } from "./use-camera";
import {
  useFaceDetection,
  FACE_GUIDANCE_TEXT,
  headTurnState,
  faceSizeVariance,
  type FaceBox,
} from "./use-face-detection";
import { analyzeFrame, captureFrame } from "./capture-utils";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import type { FaceCheckResult } from "@/types";

type Phase = "position" | "liveness" | "done";

/** Нүүрний 3 байрлал — урд (center), баруун, зүүн */
type PoseKind = "center" | "right" | "left";

interface LivenessStep {
  kind: PoseKind;
  label: string;
}

/** 3 байрлал — урд → баруун → зүүн. Тус бүрд автоматаар зураг авна. */
const POSES: LivenessStep[] = [
  { kind: "center", label: "Камер руу эгц харж, хөдөлгөөнгүй барина уу." },
  { kind: "right", label: "Толгойгоо баруун тийш эргүүлнэ үү." },
  { kind: "left", label: "Толгойгоо зүүн тийш эргүүлнэ үү." },
];
/** Арын нийцлийн дотоод нэр — `LIVENESS_STEPS` хуучин нэр дуусахгүй */
const LIVENESS_STEPS = POSES;

const HOLD_MS = 700;
const MIN_POSE_FRAMES = 3;
/** Нэг алхам хэт удаан зогссон үед хэрэглэгчид дохио өгөх — гацахаас сэргийлнэ */
const STEP_TIMEOUT_MS = 8000;

/** Нэг pose-ийг дагаж хадгалагдсан зургийн бүртгэл */
interface PoseCapture {
  pose: "front" | "left" | "right";
  blob: Blob;
  dataUrl: string;
}

/** FaceCapture-ын onComplete буцаалт — 3 байрлалын зургууд (poses) + арын нийцэл home blob */
export interface FaceCaptureResult {
  blob: Blob; // home = урд (front) — backward compat
  dataUrl: string;
  faceResult: FaceCheckResult;
  poses: PoseCapture[];
}

/**
 * Front camera + real-time face guidance + 3-байрлалт capture (урд/баруун/зүүн) + liveness.
 * Biometric identity matching ХИЙДЭГГҮЙ, зөвхөн detection/liveness/anti-spoof.
 */
export function FaceCapture({
  onComplete,
  onCancel,
}: {
  onComplete: (result: FaceCaptureResult) => void;
  onCancel: () => void;
}) {
  const { videoRef, status, start } = useCamera("user");
  const [brightness, setBrightness] = useState<number | null>(null);
  const [phase, setPhase] = useState<Phase>("position");
  const [stepIndex, setStepIndex] = useState(0);
  const [holdSince, setHoldSince] = useState<number | null>(null);
  const [holdProgress, setHoldProgress] = useState(0);
  const [stalled, setStalled] = useState(false);
  const poseFrameCountRef = useRef(0);

  // Mount хийгдмэгц камерыг автоматаар эхлүүлнэ
  useEffect(() => {
    start();
  }, [start]);

  const { faces, occluded, guidance, backend, widthHistoryRef } = useFaceDetection(
    videoRef,
    brightness,
    status === "active" && phase !== "done"
  );

  // Refs — stale сlosure-оос сэргийлж state-machine-ийн бүх төлөвийг энд хадгална
  const prevFaceRef = useRef<FaceBox | null>(null);
  const stepResultsRef = useRef<boolean[]>([]);
  const firstStepTimeRef = useRef<number>(-1);
  const lastStepTimeRef = useRef<number>(-1);
  const lastStepAtRef = useRef<number>(-1);
  const colorSamplesRef = useRef<{ r: number; g: number; b: number }[]>([]);
  // 3 байрлалын зургийн цуглуулга — pose бүрийн capture хийгдэх үед нэмэгдэнэ
  const poseCapturesRef = useRef<PoseCapture[]>([]);
  const occludedRef = useRef(false);
  occludedRef.current = occluded;
  const completedRef = useRef(false);
  const phaseRef = useRef<Phase>("position");
  const stepIndexRef = useRef(0);
  phaseRef.current = phase;
  stepIndexRef.current = stepIndex;

  // Stalled hint — нэг алхам дээр удаан зогссон цагт дохио өгнө (гацахаас сэргийлэх)
  useEffect(() => {
    if (status !== "active" || phase === "done") return;
    const id = window.setInterval(() => {
      if (holdSince !== null) {
        setStalled(performance.now() - holdSince > STEP_TIMEOUT_MS);
      } else {
        setStalled(false);
      }
    }, 1000);
    return () => window.clearInterval(id);
  }, [status, phase, holdSince]);

  // Brightness + color sampling loop (300ms)
  useEffect(() => {
    if (status !== "active" || phase === "done") return;
    let raf: number | null = null;
    let last = 0;
    const loop = () => {
      const now = performance.now();
      const video = videoRef.current;
      if (video && video.readyState >= 2 && video.videoWidth > 0 && now - last > 300) {
        last = now;
        const a = analyzeFrame(video);
        if (a) {
          setBrightness(a.brightness);
          // Зөвхөн pose-г барьж буй hold үед дээж авна. Эхний байрлал/pose солилт
          // болон auto-exposure-ийн өөрчлөлтийг color consistency-д хольж болохгүй.
          if (holdSince !== null) {
            colorSamplesRef.current.push({ r: a.avgR, g: a.avgG, b: a.avgB });
            if (colorSamplesRef.current.length > 40) colorSamplesRef.current.shift();
          }
        }
      }
      raf = requestAnimationFrame(loop);
    };
    raf = requestAnimationFrame(loop);
    return () => {
      if (raf) cancelAnimationFrame(raf);
    };
  }, [status, phase, holdSince, videoRef]);

  // Hold progress animation
  useEffect(() => {
    if (holdSince === null) {
      setHoldProgress(0);
      return;
    }
    let raf: number | null = null;
    const loop = () => {
      const p = Math.min(1, (performance.now() - holdSince) / HOLD_MS);
      setHoldProgress(p);
      if (p < 1) raf = requestAnimationFrame(loop);
    };
    raf = requestAnimationFrame(loop);
    return () => {
      if (raf) cancelAnimationFrame(raf);
    };
  }, [holdSince]);

  // finishCapture — бүх төлөвийг ref-ээс уншина (stale closure асуудалгүй)
  const finishCapture = useCallback((mode: "auto" | "manual" = "auto") => {
    if (completedRef.current) return; // double-fire хамгаалалт
    const video = videoRef.current;
    const capturedPoses = poseCapturesRef.current;

    // home blob/dataUrl — 3 pose-ийн урд (front) хэсэг, эсвэл captureFrame-ээс нэг frame
    let blob: Blob;
    let dataUrl: string;
    if (capturedPoses.length > 0) {
      const home = capturedPoses.find((p) => p.pose === "front") ?? capturedPoses[0]!;
      blob = home.blob;
      dataUrl = home.dataUrl;
    } else if (video && video.videoWidth !== 0) {
      try {
        const c = captureFrame(video, 1000, 0.8);
        blob = c.blob;
        dataUrl = c.dataUrl;
      } catch {
        // Зураг авахад алдаа — бүтэлгүйтэл гэж үзнэ
        completedRef.current = true;
        setPhase("done");
        window.setTimeout(() => {
          onComplete({
            blob: new Blob([], { type: "image/jpeg" }),
            dataUrl: "",
            faceResult: {
              passed: false,
              livenessPassed: false,
              checks: {
                faceDetected: false,
                singleFace: false,
                lightingOk: false,
                centered: false,
                stepsCompleted: 0,
                steps: [],
                blinkDetected: false,
                sizeVariance: 0,
                colorConsistent: false,
                totalElapsedMs: 0,
                confidence: 0,
                poseCaptures: 0,
                occluded: false,
              },
              note: "Зураг авахад алдаа гарлаа — admin хяналт шаардлагатай.",
            },
            poses: [],
          });
        }, 1300);
        return;
      }
    } else {
      // Видео бэлэн биш — бүтэлгүйтэл
      completedRef.current = true;
      setPhase("done");
      window.setTimeout(() => {
        onComplete({
          blob: new Blob([], { type: "image/jpeg" }),
          dataUrl: "",
          faceResult: {
            passed: false,
            livenessPassed: false,
            checks: {
              faceDetected: false,
              singleFace: false,
              lightingOk: false,
              centered: false,
              stepsCompleted: 0,
              steps: [],
              blinkDetected: false,
              sizeVariance: 0,
              colorConsistent: false,
              totalElapsedMs: 0,
              confidence: 0,
              poseCaptures: 0,
              occluded: false,
            },
            note: "Зураг авахад алдаа гарлаа — admin хяналт шаардлагатай.",
          },
          poses: [],
        });
      }, 1300);
      return;
    }

    const detected = facesRef.current.length > 0;
    const stepsDone = stepResultsRef.current;
    const totalSteps = LIVENESS_STEPS.length;
    // Бүх 3 pose-ийн зураг бүрэн авагдсан эсэх — liveness давтахад шаардлагатай
    const didAllSteps = phaseRef.current === "liveness" && capturedPoses.length >= totalSteps;
    const sizeVar = faceSizeVariance(widthHistoryRef.current);

    // Өнгө тогтвортой байдал (video replay-ээс сэргийлэх)
    const samples = colorSamplesRef.current;
    let colorConsistent = true;
    if (samples.length >= 2 && samples[0]) {
      const s0 = samples[0];
      for (const s of samples) {
        if (Math.abs(s.r - s0.r) > 18 || Math.abs(s.g - s0.g) > 18 || Math.abs(s.b - s0.b) > 18) {
          colorConsistent = false;
          break;
        }
      }
    }

    // Anti-spoof нийлмэл шалгалт — ТОЛЬКО бодит алхмууд хийгдсэн үед л liveness давагдсан гэж үзнэ.
    // Manual/override үед livenessPassed=false (шүүмжлэлтэй хэвээр), гэхдээ зураг admin руу очно.
    const livenessPassed =
      backendRef.current === "landmarker" &&
      didAllSteps &&
      stepsDone.length >= totalSteps &&
      stepsDone.every(Boolean) &&
      sizeVar >= 0.02 &&
      colorConsistent &&
      !occludedRef.current;

    const totalElapsedMs =
      firstStepTimeRef.current > 0 && lastStepTimeRef.current > 0
        ? lastStepTimeRef.current - firstStepTimeRef.current
        : 0;

    const checks: FaceCheckResult["checks"] = {
      faceDetected: detected,
      singleFace: facesRef.current.length === 1,
      lightingOk: brightnessRef.current !== null && brightnessRef.current >= 62 && brightnessRef.current <= 205,
      centered: guidanceRef.current === "ok",
      stepsCompleted: stepsDone.filter(Boolean).length,
      steps: didAllSteps ? stepsDone.slice(0, totalSteps) : stepsDone,
      blinkDetected: false,
      sizeVariance: sizeVar,
      colorConsistent,
      totalElapsedMs,
      confidence: 0,
      poseCaptures: capturedPoses.length,
      occluded: occludedRef.current,
    };
    checks.confidence =
      0.3 * (checks.faceDetected ? 1 : 0) +
      0.2 * (checks.singleFace ? 1 : 0) +
      0.2 * (checks.lightingOk ? 1 : 0) +
      0.15 * (checks.centered ? 1 : 0) +
      0.15 * (didAllSteps ? 1 : 0) -
      (checks.occluded ? 0.1 : 0);

    const result: FaceCheckResult = {
      // Зургийг admin руу илгээхэд ашиглах боломжтой гэдгийг passed илэрхийлнэ
      passed: mode === "manual" || backendRef.current === "none" ? false : livenessPassed,
      livenessPassed,
      checks,
      note:
        mode === "manual" || backendRef.current === "none"
          ? "Нүүр илрүүлэлт хязгаарлагдмал — admin хяналт шийднэ."
          : stalledRef.current
            ? "Алхам хэт удаан зогссон — admin хяналт шаардлагатай."
            : checks.occluded
              ? "Нүүр халхалсан байсан — admin хяналт шаардлагатай."
              : null,
    };

    completedRef.current = true;
    setPhase("done");
    window.setTimeout(
      () =>
        onComplete({
          blob,
          dataUrl,
          faceResult: result,
          poses: capturedPoses.map((p) => ({
            pose: p.pose,
            blob: p.blob,
            dataUrl: p.dataUrl,
          })),
        }),
      1300
    );
  }, [videoRef, widthHistoryRef, onComplete]);

  /**
   * ГАР АВАХ — ямар ч үед (backend ямар ч бай, ямар ч phase) зургийг аваад дуусгана.
   * Автомат детектор гацсан/дэмжигдэхгүй үед ч зураг ямагт авагдана.
   */
  const manualCapture = useCallback(() => {
    if (completedRef.current || phaseRef.current === "done") return;
    const video = videoRef.current;
    // Liveness дунд байвал одоогийн алхмын pose-ийн зургийг хадгалж, дараа нь дуусгана
    if (phaseRef.current === "liveness") {
      const step = LIVENESS_STEPS[stepIndexRef.current] ?? null;
      if (step) {
        stepResultsRef.current[stepIndexRef.current] = true;
        lastStepTimeRef.current = performance.now();
        if (video && video.videoWidth > 0) {
          try {
            const c = captureFrame(video, 1000, 0.8);
            const poseLabel = step.kind === "center" ? "front" : step.kind;
            if (!poseCapturesRef.current.some((p) => p.pose === poseLabel)) {
              poseCapturesRef.current.push({
                pose: poseLabel,
                blob: c.blob,
                dataUrl: c.dataUrl,
              });
            }
          } catch {
            /* capture алдаа — pose-гүйгээр үргэлжлүүлнэ */
          }
        }
      }
    }
    finishCapture("manual");
  }, [finishCapture, videoRef]);

  // Render үед refs-ийг шинэчилнэ — finishCapture-д хамгийн сүүлийн утгууд очно
  const facesRef = useRef(faces);
  facesRef.current = faces;
  const guidanceRef = useRef(guidance);
  guidanceRef.current = guidance;
  const brightnessRef = useRef(brightness);
  brightnessRef.current = brightness;
  const backendRef = useRef(backend);
  backendRef.current = backend;
  const stalledRef = useRef(stalled);
  stalledRef.current = stalled;

  // Main state machine — face guidance + 3-байрлалт pose capture
  useEffect(() => {
    if (status !== "active" || phase === "done") return;

    const face = faces[0] ?? null;
    const step = LIVENESS_STEPS[stepIndex] ?? null;
    let conditionMet = false;

    if (faces.length !== 1) {
      conditionMet = false;
    } else if (phase === "position") {
      // Эхлээд нүүр төвд зөв байрлалд орохыг хүлээнэ ("урд" pose-ийн эхлэл)
      conditionMet = guidance === "ok";
    } else if (step) {
      if (step.kind === "center") conditionMet = guidance === "ok";
      else conditionMet = headTurnState(face, prevFaceRef.current) === step.kind;
    }

    if (face) prevFaceRef.current = face;

    if (conditionMet) {
      poseFrameCountRef.current += 1;
      if (holdSince === null) {
        setHoldSince(performance.now());
        if (firstStepTimeRef.current < 0) firstStepTimeRef.current = performance.now();
      } else if (performance.now() - holdSince >= HOLD_MS && poseFrameCountRef.current >= MIN_POSE_FRAMES) {
        const now = performance.now();
        lastStepTimeRef.current = now;
        if (phase === "position") {
          // Baseline-ийг phase солихоос өмнө хадгална. Ингэснээр шинэ
          poseFrameCountRef.current = 0;
          setHoldSince(null);
          setPhase("liveness");
          setStepIndex(0);
          prevFaceRef.current = null;
        } else if (step) {
          // Pose зөв тогтсон үед зураг амжилттай авсны дараа л дараагийн pose руу шилжинэ.
          // Ингэснээр canvas/video frame түр бэлэн биш үед 3 зургаас нэг нь алдагдахгүй.
          const video = videoRef.current;
          const poseLabel = step.kind === "center" ? "front" : step.kind;
          let captured = false;
          if (video && video.videoWidth > 0) {
            try {
              const c = captureFrame(video, 1000, 0.8);
              if (!poseCapturesRef.current.some((p) => p.pose === poseLabel)) {
                poseCapturesRef.current.push({ pose: poseLabel, blob: c.blob, dataUrl: c.dataUrl });
              }
              captured = true;
            } catch {
              // Дараагийн frame дээр ижил pose-г дахин тогтоож авна.
            }
          }
          if (!captured) {
            setHoldSince(null);
            poseFrameCountRef.current = 0;
            return;
          }
          stepResultsRef.current[stepIndex] = true;
          lastStepAtRef.current = now;
          setHoldSince(null);
          poseFrameCountRef.current = 0;
          if (stepIndex < LIVENESS_STEPS.length - 1) {
            setStepIndex((i) => i + 1);
            prevFaceRef.current = null;
          } else {
            // Сүүлийн pose — бүрэн дуусгана
            finishCapture("auto");
          }
        }
      }
    } else {
      // Pose таарахгүй / нүүр халхалсан (covered) — hold reset (auto-retake)
      setHoldSince(null);
      poseFrameCountRef.current = 0;
      prevFaceRef.current = null;
    }
  }, [status, phase, faces, guidance, stepIndex, holdSince, finishCapture]);

  const currentStep = LIVENESS_STEPS[stepIndex];
  const livenessInstruction = currentStep?.label;
  // Холд progress ring — state machine-тэй ижил нөхцөл (бусад алхмуудад ч харагдана)
  const activeCondition =
    phase === "position"
      ? guidance === "ok"
      : currentStep?.kind === "center"
        ? guidance === "ok"
        : faces.length === 1 && !!currentStep &&
          headTurnState(faces[0] ?? null, prevFaceRef.current) === currentStep.kind;
  const showHold = activeCondition && holdSince !== null;
  const holdCount = showHold ? Math.max(1, Math.ceil((1 - holdProgress) * 3)) : null;

  return (
    <div className="safe-pt safe-pb fixed inset-0 z-[70] flex flex-col bg-[#070b18]">
      <div className="relative z-20 flex min-h-14 items-center gap-3 border-b border-white/10 px-4 py-2">
        <button
          onClick={onCancel}
          aria-label="Буцах"
          className="focus-ring flex h-11 w-11 items-center justify-center rounded-xl bg-white/8 text-white transition hover:bg-white/15"
        >
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-white">Нүүрний баталгаажуулалт</p>
          <p className="truncate text-xs text-zinc-400">
            {phase === "position" ? "Нүүрээ хүрээний төвд байрлуулна уу." : livenessInstruction}
          </p>
        </div>
        <ScanFace className="h-5 w-5 text-white" />
      </div>

      <div className="relative flex-1 overflow-hidden">
        {(status === "idle" || status === "requesting") ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-zinc-950/75 backdrop-blur-sm" role="status" aria-live="polite">
            <div className="flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-zinc-100">
              <Loader2 className="h-4 w-4 animate-spin text-white" /> Камер нээгдэж байна…
            </div>
          </div>
        ) : null}
        {status === "denied" || status === "unavailable" || status === "error" || status === "inuse" ? (
          <FaceProblem status={status} onRetry={start} />
        ) : phase === "done" ? (
          <div className="flex h-full flex-col items-center justify-center gap-5 px-8 text-center">
            <span className="relative flex h-24 w-24 items-center justify-center">
              <span className="absolute inset-0 rounded-full bg-white/15 animate-pulse-ring" />
              <span className="absolute inset-0 rounded-full bg-white/10 animate-pulse-ring [animation-delay:0.5s]" />
              <CheckCircle2 className="relative h-14 w-14 text-white" />
            </span>
            <h3 className="text-xl font-bold text-white">Нүүрний шалгалт дууслаа ✓</h3>
            {poseCapturesRef.current.length >= 3 ? (
              <div className="flex items-end justify-center gap-3">
                {poseCapturesRef.current.map((p) => {
                  const label = p.pose === "front" ? "Урд" : p.pose === "left" ? "Зүүн" : "Баруун";
                  return (
                    <div key={p.pose} className="flex flex-col items-center gap-1.5">
                      <img
                        src={p.dataUrl}
                        alt={label}
                        className={cn(
                          "rounded-lg border border-white/15 object-cover",
                          p.pose === "front" ? "h-24 w-24" : "h-16 w-16"
                        )}
                      />
                      <span className="text-[10px] font-medium text-zinc-400">{label}</span>
                    </div>
                  );
                })}
              </div>
            ) : null}
            <p className="text-sm text-zinc-400">Зураг авагдаж байна…</p>
          </div>
        ) : (
          <>
            <video ref={videoRef} playsInline muted className="h-full w-full object-cover" />
            <div
              className="pointer-events-none absolute inset-0"
              style={{
                background:
                  "linear-gradient(180deg, rgba(7,11,24,0.76) 0%, transparent 24%, transparent 68%, rgba(7,11,24,0.94) 100%)",
              }}
            >
              <div
                className="absolute inset-0"
                style={{
                  background:
                    "radial-gradient(ellipse at center, transparent 34%, rgba(7,11,24,0.78) 100%)",
                }}
              />
              {/* Oval guide */}
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="relative h-[54%] w-[68%] max-w-sm">
                  <div
                    className={cn(
                      "absolute inset-0 rounded-[50%] border-2 transition-all duration-300",
                      activeCondition
                        ? "border-white shadow-[0_0_44px_rgba(255,255,255,0.24)]"
                        : "border-white/60"
                    )}
                  />
                  {showHold ? (
                    <>
                      <div
                        className="absolute -inset-1.5 rounded-[50%] border-2 border-white/70"
                        style={{
                          clipPath: `inset(0 ${100 - holdProgress * 100}% 0 0)`,
                          transform: "rotate(-90deg)",
                        }}
                      />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="flex h-16 w-16 items-center justify-center rounded-full border border-white/30 bg-black/35 text-3xl font-bold tabular-nums text-white backdrop-blur-sm" aria-live="off">
                          {holdCount}
                        </span>
                      </div>
                    </>
                  ) : null}
                </div>
              </div>
            </div>
          </>
        )}
      </div>

      {status === "active" && phase !== "done" ? (
        <div className="relative z-20 px-6 pb-8 pt-4">
          {/* Liveness progress dots */}
          {phase === "liveness" ? (
            <div className="mb-3 flex items-center justify-center gap-2">
              {LIVENESS_STEPS.map((_, i) => (
                <span
                  key={i}
                  className={cn(
                    "h-2 rounded-full transition-all duration-300",
                    i < stepIndex
                      ? "w-6 bg-white"
                      : i === stepIndex
                        ? "w-4 bg-white/60"
                        : "w-2 bg-white/15"
                  )}
                />
              ))}
            </div>
          ) : null}
          <div
            className={cn(
              "mx-auto flex w-fit max-w-full items-center gap-2 rounded-2xl border px-4 py-2.5 text-sm font-medium backdrop-blur-md transition-all duration-300",
              activeCondition
                ? "border-white/30 bg-white/15 text-white"
                : "border-white/15 bg-zinc-900/80 text-zinc-200",
              (stalled || guidance === "covered") &&
                "border-amber-400/40 bg-amber-500/15 text-amber-300"
            )}
            role="status"
            aria-live="polite"
          >
            <ScanFace className="h-4 w-4 shrink-0" />
            <span className="truncate">
              {guidance === "covered"
                ? FACE_GUIDANCE_TEXT["covered"]
                : stalled
                  ? "Толгойгоо зөв байрлалд тогтвортой барина уу…"
                  : phase === "liveness"
                    ? livenessInstruction
                    : FACE_GUIDANCE_TEXT[guidance]}
            </span>
          </div>
          <div className="mt-4 flex min-h-7 items-center justify-center">
            <p className={cn("max-w-sm text-center text-xs text-zinc-500 transition-opacity", stalled ? "opacity-100" : "opacity-60")}>
              {stalled ? "Илрүүлэлт удааширлаа. Нүүрээ хүрээнд тогтвортой барина уу." : "Зөв байрлалд ормогц автоматаар зураг авна."}
            </p>
          </div>
        </div>
      ) : null}
    </div>
  );
}

function FaceProblem({ status, onRetry }: { status: CameraStatus; onRetry: () => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 px-8 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-rose-500/10">
        <Camera className="h-8 w-8 text-rose-400" />
      </div>
      <h3 className="text-lg font-semibold text-white">Камер ашиглах боломжгүй</h3>
      <p className="max-w-sm text-sm leading-relaxed text-zinc-400">{cameraErrorMessage(status)}</p>
      <Button onClick={onRetry}>
        <RefreshCcw className="h-4 w-4" /> Дахин оролдох
      </Button>
    </div>
  );
}
