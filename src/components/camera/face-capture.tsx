    ...
    if (conditionMet) {
      if (holdSince === null) {
        ...
        if (phase === "position") {
          ...
          // Pose зөв тогтсон — тухайн pose-ийн зургийг capture хийж хадгална
          ...
          if (video && video.videoWidth > 0) {
            try {
              const c = captureFrame(video, 1000, 0.8);
              const poseLabel = step.kind === "center" ? "front" : step.kind;
              // Дахин нэг pose-ийн давхар зургаас сэргийлнэ (аль хэдийн авсан pose-ийг дахин авалгүй)
              if (!poseCapturesRef.current.some((p) => p.pose === poseLabel)) {
                poseCapturesRef.current.push({
                  pose: poseLabel,
                  blob: c.blob,
              ...
              /* capture алдаа — дараагийн шаагүй, pose-ийг хадгалахгүй */
            }
          }
          setHoldSince(null);
    ...
    <div className="fixed inset-0 z-[70] flex flex-col bg-ink-950">
              ...
              <div className="flex items-end justify-center gap-3">
                {poseCapturesRef.current.map((p) => (
                  <img
                    key={p.pose}
                    src={p.dataUrl}
                    alt={p.pose === "front" ? "Урд" : p.pose === "left" ? "Зүүн" : "Баруун"}
                    className={cn(
      ...
      {status === "active" && phase !== "done" ? (
        <div className="relative z-20 px-6 pb-8 pt-4">
          ...
          {/* Гар авах товч — ЯМАР Ч ТӨЛӨВТ харагдана. Автомат детектор гацсан/
              дэмжигдэхгүй/удаан үед хэрэглэгч гараар зураг авч дуусгаж чадна. */}
          <div className="mt-4 flex flex-col items-center gap-2">
            <p className="max-w-sm text-center text-xs text-zinc-500">
              Автомат илрүүлэлт удааширсан бол эсвэл түргэн гүйцээхийг хүсвэл:
            </p>
            <Button variant="secondary" onClick={manualCapture}>
              <Camera className="h-4 w-4" /> Зургаа авах
            </Button>
          </div>
        </div>
      ) : null}
