...
export function VerificationWizard() {
  ...
  const shareLocation = async () => {
    ...
    try {
      ...
      const consentJson = await consentRes.json();
      if (!consentJson.ok || typeof consentJson.data?.consentId !== "string" || !consentJson.data.consentId) {
        throw new Error(consentJson.error ?? "Байршлын зөвшөөрөл бүртгэгдсэнгүй.");
      }
      const locConsentId = consentJson.data.consentId as string;
    ...
    <div className="mx-auto max-w-2xl">
            ...
            <Card>
              <CardContent className="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" onClick={() => { setDocumentCapture(null); }} disabled={docUploading}>
                    Дахин авах
                  </Button>
              ...
              <CardContent className="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <div className="space-y-1">
                  <p className={cn(
                    ...
                    faceResult?.livenessPassed ? "text-brand-600 dark:text-brand-300" : "text-amber-600 dark:text-amber-300"
                  )}>
                    <CheckCircle2 className="h-4 w-4" />
                    {faceResult?.livenessPassed ? "Нүүрний шалгалт дууслаа" : "Нүүрний зураг хүлээн авлаа — admin шалгана"}
                  </p>
                  <p className="text-xs text-zinc-400">
                    Амьд нүүр: {faceResult?.livenessPassed ? "Амжилттай" : "Шалгагдаагүй"} · Алхам: {faceResult?.checks.stepsCompleted ?? 0}/3
                  </p>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100 dark:bg-white/10">
                    <ScanFace className="h-5 w-5 text-zinc-900 dark:text-white" />
                  </div>
                ...
                <ul className="space-y-2 text-sm text-zinc-500 dark:text-zinc-400">
                  <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-zinc-900 dark:text-white" /> Гэрэлтэй орчинд байх</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-zinc-900 dark:text-white" /> Нүдний шил, малгайгаа тайлах</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-zinc-900 dark:text-white" /> Камерт зөвхөн та нэг хүн харагдах</li>
                </ul>
                <p className="text-xs text-zinc-400">
      ...
      {step === 4 ? (
        <div className="space-y-4 animate-fade-up">
          ...
          <WizardNav
            onBack={() => setStep(3)}
            onNext={() => setStep(5)}
            nextLabel="Хяналт руу"
            nextDisabled={locationState === "sharing"}
            hint={locationState === "sharing" ? "Байршил тодорхойлж байна…" : undefined}
          />
        </div>
      ) : null}
      ...
      {cameraOpen === "document" && documentChoice ? (
        <DocumentCapture
          ...
          aspectRatio={documentChoice === "id-card" ? 85.6 / 54 : undefined}
