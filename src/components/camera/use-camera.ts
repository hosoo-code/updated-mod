...
export function useCamera(facingMode: "environment" | "user" = "environment") {
  ...
  useEffect(() => {
    return () => {
      // Permission prompt pending үед unmount болсон бол ирэх stream-ийг хүчингүй болгоно.
      ++genRef.current;
      if (streamRef.current) {
        streamRef.current.getTracks().forEach((track) => track.stop());
        streamRef.current = null;
...
export function cameraErrorMessage(status: CameraStatus): string | null {
  switch (status) {
    case "denied":
      return "Камерын зөвшөөрөл татгалзсан байна. Энэ хуудсыг HTTPS хаягаар нээгээд browser-ийн Site settings → Camera хэсгээс зөвшөөрөл өгч, дахин оролдоно уу.";
    case "unavailable":
    ...
    case "error":
      return "Камер ачаалахад алдаа гарлаа. Камер ашиглаж байгаа бусад tab-аа хаагаад дахин оролдоно уу.";
    default:
      return null;
  }
}
