...
export function autoCropDocument(
  ...
  let crop = detectDocumentRect(video, vw, vh);
  if (crop && ratio) {
    // Илрүүлсэн тэгш өнцөгтийг баримтын харьцаанд тааруулна
    crop = fitRectToAspect(crop, ratio, vw, vh);
  }
  if (!crop) {
    if (ratio) {
      // Илрүүлэлт найдваргүй — төвөөс тухайн харьцаагаар тасарна (ID загвар)
      const cw = Math.min(vw, vh * ratio);
      const ch = cw / ratio;
      crop = {
  ...
  // Rect-ийг эх video координатад өргөтгөж, жижиг padding нэмнэ (ирмэг тасрахгүй)
  const pad = Math.round(Math.min(crop.w, crop.h) * 0.03);
