...
export interface FrameAnalysis {
  ...
  /** Edge bounding box-ийн өндөр (frame-ийн %) — баримт босоо тасарсан эсэхийг илрүүлэх */
  edgeBBoxHeight: number;
...
export function analyzeFrame(
  ...
  const edgeBBoxHeight = strongCount > 4 ? ((maxY - minY) / sh) * 100 : 0;
...
/**
 * AUTO-CROP: баримтын (иргэний үнэмлэх/гэрчилгээ) ирмэгийг илрүүлж,
 * зөвхөн тухайн хэсгийг тайрч (захын ширээ/гар/хана хаягдлана), JPEG болгоно.
 *
 * Алгоритм:
 *  1. Frame-ийг бага resolution-т буулгаж gray + Sobel edge detection хийнэ.
 *  2. Context-aware босго (median-based) → баримтын ирмэгийн binary mask.
 *  3. Morphological closing (цоорхойг хаах) → холбоотой контур.
 *  4. Хамгийн том контурын bounding rect-ийг олж, төвийн баримт мөн эсэхийг шалгана.
 *  5. Олдсон rect-ийг жижиг padding-тайгаар бодит video-оос крап хийж JPEG болгоно.
 *  6. Найдвартай илрүүлээгүй бол (баримт төвд байхгүй / хэт жижиг) бүтэн frame буцаана.
 *
 * opts.aspectRatio өгвөл (жишээ: иргэний үнэмлэх 85.6/54 ≈ 1.585) — зургийг
 * тухайн харьцааны тэгш өнцөгтөд заавал тааруулж, илрүүлэлт амжилтгүй бол
 * төвийн хэсгээс мөн тухайн харьцаагаар тасарна. Ингэснээр авсан зураг
 * үнэмлэхийн жинхэнэ хэлбэртэй (тэгш өнцөгт credit-card харьцаа) байна.
 *
 * Perspective warp ЭНЭ хувилбарт ороогүй — тэгш өнцөгт crop нь хамгийн найдвартай.
 * Яагаад: ID илрүүлэлт нь гэрэл/өнцгөөс маш их хамааралтай, буруу warp алдаатай
 * зургийг илгээхээс илүү тэгш өнцөгт crop аюулгүй.
 */
export function autoCropDocument(
  video: HTMLVideoElement,
  maxSize = 1800,
  quality = 0.95,
  opts?: { aspectRatio?: number }
): { dataUrl: string; blob: Blob; width: number; height: number } {
  const vw = video.videoWidth;
  const vh = video.videoHeight;
  if (!vw || !vh) throw new Error("Зураг авахад алдаа гарлаа. Камераа шалгаад дахин оролдоно уу.");

  const ratio = opts?.aspectRatio;
  let crop: { x: number; y: number; w: number; h: number } | null;
  if (ratio && ratio > 0) {
    // ID-1 картын хувьд edge component-д найдаж болохгүй. Текст/зурагны
    // дотоод ирмэгийг document гэж буруу танивал картын хоёр тал тасардаг.
    // Камерын голд байгаа guide frame-тэй ижил, яг ID-1 харьцааны crop ашиглана.
    const frameScale = 0.94;
    const cw = Math.min(vw * frameScale, vh * frameScale * ratio);
    const ch = cw / ratio;
    crop = {
    ...
    crop = detectDocumentRect(video, vw, vh);
    if (!crop) {
      // Илрүүлэлт найдваргүй — бүтэн frame-аар буцаана (fallback)
      return captureFrame(video, maxSize, quality);
    }
  }

  // ID картын crop-д padding нэмэхгүй: padding хийвэл гаралтын харьцаа
  // дахин өөрчлөгдөж, картын талууд яг ID-1 хэлбэрээр гарахгүй.
  const pad = ratio ? 0 : Math.round(Math.min(crop.w, crop.h) * 0.03);
  let sx = Math.max(0, crop.x - pad);
  let sy = Math.max(0, crop.y - pad);
  let sw = Math.min(vw - sx, crop.w + pad * 2);
  let sh = Math.min(vh - sy, crop.h + pad * 2);
  if (ratio) {
    const adjusted = fitRectToAspect({ x: sx, y: sy, w: sw, h: sh }, ratio, vw, vh);
    sx = adjusted.x;
    sy = adjusted.y;
    sw = adjusted.w;
    sh = adjusted.h;
  }
  if (sw <= 100 || sh <= 100) return captureFrame(video, maxSize, quality);

  // Дээд resolution-т крап хийнэ
...
/**
 * Edge-based баримт илрүүлэлт — баримтын тэгш өнцөгт байршлыг буцаана.
 * @returns {x,y,w,h} эх video координатад, баримт олдоогүй бол null.
 */
function detectDocumentRect(
  ...
  // Шинжилгээний resolution — алгоритм найдвартай, хурдан ажиллана
  const S = 320;
  const scaleS = S / Math.max(vw, vh);
  const sw = Math.max(2, Math.round(vw * scaleS));
  const sh = Math.max(2, Math.round(vh * scaleS));
  ...
  const meanMag = magSum / ((sh - 2) * (sw - 2));
  // Context-aware босго: дунджаас хэд дахин их edge-үүдийг л баримтын ирмэг гэж үзнэ
  const thresh = Math.max(12, meanMag * 1.5);

  // Connected component (BFS) — хамгийн том холбоотой блок олох
  const visited = new Uint8Array(sw * sh);
  let best: { x0: number; y0: number; x1: number; y1: number; area: number } | null = null;
  const queue = new Int32Array(sw * sh);

  const isStrong = (x: number, y: number) => (edge[y * sw + x] ?? 0) > thresh;

  for (let start = 0; start < sw * sh; start++) {
    if (visited[start] || !isStrong(start % sw, (start / sw) | 0)) continue;
    // BFS
    let head = 0;
    let tail = 0;
    queue[tail++] = start;
    ...
    while (head < tail) {
      ...
      if (y > maxY) maxY = y;
      count++;
      // 4-малж эсвэл 8-малж — 8-малж нь тасралтгүй ирмэгийг сайн нэгтгэнэ
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          const nIdx = ny * sw + nx;
          if (visited[nIdx] || !isStrong(nx, ny)) continue;
          visited[nIdx] = 1;
          queue[tail++] = nIdx;
        }
      }
    }
    if (count > 30 && count > (best?.area ?? 0)) {
      best = { x0: minX, y0: minY, x1: maxX, y1: maxY, area: count };
    }
  }

  if (!best || best.area < 40) return null;

  const bw = best.x1 - best.x0;
  const bh = best.y1 - best.y0;
  if (bw < 40 || bh < 25) return null;

  // Баримт төвд байх ёстой — ирмэгийн блок төвд хандсан эсэхийг шалгана
  const centerX = (best.x0 + best.x1) / 2 / sw;
  const centerY = (best.y0 + best.y1) / 2 / sh;
  if (centerX < 0.3 || centerX > 0.7 || centerY < 0.3 || centerY > 0.7) return null;

  return {
    x: Math.round((best.x0 / sw) * vw),
    y: Math.round((best.y0 / sh) * vh),
    w: Math.round((bw / sw) * vw),
    h: Math.round((bh / sh) * vh),
  };
}
...
/**
 * Тэгш өнцөгтийг тухайн aspect ratio-д тааруулна — голд нь төвлөрүүлж,
 * хэт сунасан талыг тасарна. Жишээ: иргэний үнэмлэх 85.6/54 ≈ 1.585.
 */
export function fitRectToAspect(
  ...
  if (current > aspectRatio) {
    // Хэт өргөн — өндрийг нэмэгдүүлж харьцаанд оруулна (дээр/доор тэлнэ)
    const newH = w / aspectRatio;
    y = y - (newH - h) / 2;
    h = newH;
  } else {
    // Хэт өндөр — өргөнийг нэмэгдүүлж харьцаанд оруулна (зүүн/баруун тэлнэ)
    const newW = h * aspectRatio;
    x = x - (newW - w) / 2;
    w = newW;
  }
  // Frame-ийн хилээс гадуур гарахгүй байлгах — гарах юм бол дотор нь төвлөрүүлнэ
  if (w > vw) {
    w = vw;
    h = w / aspectRatio;
  }
  ...
  w = Math.min(vw - x, Math.round(w));
  h = Math.min(vh - y, Math.round(h));
  return { x, y, w, h };
}
